...
namespace sibr
{
	...
	void MultiViewBase::onUpdate(Input& input)
	{
		...
		// std::cout << "MultiViewBase::onUpdate is called" << std::endl;
		...
		for (auto & subview : _subViews) {
			if (subview.second.view->active()) {
				...
				// std::cout << "width: " << subview.second.viewport.finalSize()(0) << std::endl;
	...
	void MultiViewBase::renderSubView(SubView & subview) 
	{
		...
		if (!_onPause) {
			...
			subview.rt->bind();
			renderViewport.bind();
			subview.view->RenderHelpers(renderViewport);
			subview.rt->unbind();
	...
	void MultiViewManager::onRender(Window & win)
	{
		...
		std::map<std::string, IBRSubView>::iterator subview = _ibrSubViews.begin();

		if ((cur_frame_idx < max_frame_idx) && (cur_frame_idx != 0)) {
			captureView(subview->second, _exportPath, std::to_string(std::max(cur_frame_idx - 2, 0))+".png");
		}
		...
		// auto & subview = _ibrSubViews[0];

		if (cur_frame_idx < max_frame_idx) {
			InteractiveCameraHandler* interact_p = (subview->second.handler).get();
			interact_p->DirectSnapToCamera(cur_frame_idx);
			cur_frame_idx += 1;
		}
	...
	void MultiViewManager::onGui(Window & win)
	{
		...
		// ImGui::GetStyle().ScaleAllSizes(0.0f);
		...
		if (_showGUI && ImGui::BeginMainMenuBar())
		{
			...
			if (ImGui::BeginMenu("Capture"))
			{
				...
				for (auto & subview : _ibrSubViews) {
					if (ImGui::MenuItem((subview.first + " export frames").c_str())) {
						max_frame_idx = 202;
					}
				}
