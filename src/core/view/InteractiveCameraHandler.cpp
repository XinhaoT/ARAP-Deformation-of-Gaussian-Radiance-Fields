...
namespace sibr {
	...
	void InteractiveCameraHandler::DirectSnapToCamera(const int i) {
		if (!_interpPath.empty()) {
			unsigned int nearestCam = (i == -1 ? findNearestCamera(_interpPath) : i);
			nearestCam = sibr::clamp(nearestCam, (unsigned int)(0), (unsigned int)(_interpPath.size() - 1));
			_currentCamId = nearestCam;
			std::cout << "nearestCam: " << nearestCam << std::endl;
	...
	void InteractiveCameraHandler::update(const sibr::Input & input, float deltaTime, const sibr::Viewport & viewport) {
		...
		if (input.key().isReleased(sibr::Key::K)) {
			// switchMode(_currentMode == ORBIT ? FPS : ORBIT);
		}
		else if (input.key().isReleased(sibr::Key::L)) {
		...
		// only free key
		...
		else if (input.key().isReleased(sibr::Key::F12)) {
			switchMode(_currentMode == TRACKBALL ? FPS : TRACKBALL);
		}
		else if (input.key().isReleased(sibr::Key::F11)) {
			switchSnapping();
		}
			...
			if (input.key().isActivated(Key::LeftShift) && (input.key().isActivated(Key::LeftAlt) || input.key().isActivated(Key::LeftControl)) && input.key().isReleased(Key::F12))
			{
			...
			else if (input.key().isActivated(Key::LeftShift) && input.key().isReleased(Key::F12))
			{
				std::string filename;
			...
			else if (input.key().isActivated(Key::LeftControl) && input.key().isReleased(Key::F12))
			{
				std::string filename;
				for (uint i = 0; i < 10; ++i) std::cout << std::endl;
			...
			else if (input.key().isActivated(Key::LeftAlt) && input.key().isReleased(Key::F12))
			{
				_cameraRecorder.reset();
				_cameraRecorder.record();
			}
			else if (input.key().isActivated(Key::RightAlt) && input.key().isReleased(Key::F12)) {
				std::string filename;
				for (uint i = 0; i < 10; ++i) std::cout << std::endl;
				std::cout << "Enter a filename for saving a camera path:" << std::endl;
				safeGetline(std::cin, filename);
			...
			else if (input.key().isReleased(Key::F12)) {
				_cameraRecorder.playback();
			}
		}
