...
namespace sibr {
	...
	void TrackBall::update(const Input & input, const Viewport & viewport, std::shared_ptr<Raycaster> raycaster)
	{
		...
		if (input.key().isActivated(sibr::Key::M) || input.key().isActivated(sibr::Key::N) || input.key().isActivated(sibr::Key::B) || input.key().isActivated(sibr::Key::V) || input.key().isActivated(sibr::Key::X) || input.key().isActivated(sibr::Key::K)){
			disable_trackball = true;
		}
		else{
			disable_trackball = false;
		}

		// std::cout << "disable_trackball: " << disable_trackball << std::endl;
		if (disable_trackball) {return;}
	...
	void TrackBall::updateTrackBallStatus(const Input & input, const Viewport & viewport)
	{
		...
		if (input.key().isActivatedOnly(Key::T) && input.key().isActivatedOnly(Key::L)) {
			verbose = !verbose;
			if (verbose) {
	...
	void TrackBall::updateRadius(const Input & input)
	{
		...
		else
		{
			...
			fixedCamera.orthoRight(fixedCamera.orthoRight() * pow(1.1f, zoomIn));
			fixedCamera.orthoTop(fixedCamera.orthoTop() * pow(1.1f, zoomIn));
			zoom /= pow(1.1f, zoomIn);
		}
	}
	...
	void TrackBall::updateZnearZFar(const Input & input)
	{
		...
		if (input.key().isActivatedOnly(Key::F12)) {
			fixedCamera.zfar(fixedCamera.zfar()* pow(1.25f, direction));
			printMessage(" zFar : " + std::to_string(fixedCamera.zfar()));
		}
		else if (input.key().isActivatedOnly(sibr::Key::F12) && input.key().isActivatedOnly(Key::LeftShift)) {
