...
int main(int ac, char** av) 
{
	...
	while (window.isOpened()) 
	{
		...
		ImGui::Begin("MAIN");
		ImGui::PushScaledItemWidth(10);
		if (ImGui::Button("Load PLY file")) {
			sibr::showFilePicker(plyfile, sibr::Default);
			...
			gaussianView = GaussianView::Ptr(new GaussianView(scene, sceneResWidth, sceneResHeight, plyfile.c_str(), &messageRead, sh_degree, white_background, !myArgs.noInterop, device));

			generalCamera = sibr::InteractiveCameraHandler::Ptr(new InteractiveCameraHandler());
			generalCamera->setup(scene->cameras()->inputCameras(), Viewport(0, 0, (float)usedResolution.x(), (float)usedResolution.y()), nullptr);
